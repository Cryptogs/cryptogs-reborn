...
/**
 * @title Cryptogs
 * @author Austin Griffith(2018?), and (2022) Later Comments made by Steve P., 0xAA, COSMOBURN.eth
 * @notice SlammerTime.sol and this contract are the main components to Cryptogs 2018.
 * NOTE: The general breakdown of this contract is as follows:
 * 1.
 */
contract Cryptogs is NFT, Ownable {
     ...
     * @notice
     */
    function mint(bytes32 _image, address _owner) public onlyOwner returns (uint256) {
     ...
     * @notice
     */
    function _mint(bytes32 _image) internal returns (uint256) {
        Item memory _item = Item({ image: _image });
        uint256 newId = items.push(_item) - 1; //newId is essentially the erc721 id that is assigned to the token. It is given an Id in accordance to its new spot in the items[] array. The items[] array is an array of Structs, Item, that has different structs at each array element, which carry the specific erc721 details.
    ...
    function mintPack(
        ...
        Pack memory _pack = Pack({ tokens: tokens, price: _price });
        MintPack(
            packs.push(_pack) - 1,
            _price,
            tokens[0],
        ...
        )
    {
        ...
        return (tokenIndexToOwner[_id], image, copies);
    }
    ...
    function acceptCounterStack(bytes32 _stack, bytes32 _counterStack) public returns (bool) {
        ...
        // instantiate the slammerTimeContract locally to this function to do the transfer
        SlammerTime slammerTimeContract = SlammerTime(slammerTime);
        require(
            slammerTimeContract.startSlammerTime(
                msg.sender,
                stacks[_stack].ids,
                stacks[_counterStack].owner,
                stacks[_counterStack].ids
            )
        );
            ...
            for (uint8 i = 0; i < 10; i++) {
                if (mixedStack[_stack][i] > 0) {
                    ...
                    if (thisFlipper < (FLIPPINESS + round[_stack] * FLIPPINESSROUNDBONUS)) {
                        ...
                        done = false; // this makes sure that the logic below isn't called up to finish the game until all pogs in mixedStack[_stack][i] are zeroed out (and thus transferred by above logic).
                    }
                }
            } // SP NOTE: if done var is in the for loop, can't we have a moment where done = true but the game isn't over yet?
     ...
     * @notice
     */
    function throwSlammerEvent(
        bytes32 stack,
        address whoDoneIt,
        address otherAccount,
        uint256[10] flipArray
    ) internal {
        ThrowSlammer(
            stack,
            whoDoneIt,
